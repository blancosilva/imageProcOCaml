...
let pairs = Array.make_matrix 23 2 0.0;;

for k = 3 to 25 do
  let wc_file   = "wco"^(string_of_int k) in
  let pgm_file  = wc_file^".pgm" in
  let threshold = (float (k-1)) /. 500.0 +. 0.0001 in
  ...
  pairs.(k-3).(0) <- minus_log_num_coefs;
  pairs.(k-3).(1) <- log_error;
  Printf.fprintf data "%f %f\n" minus_log_num_coefs log_error
done;;
close_out data;;

let sum_x  = (sum_filtered_column pairs 0 identity 1 23 0.0);;
let sum_y  = (sum_filtered_column pairs 1 identity 1 23 0.0);;
let sum_x2 = (sum_filtered_column pairs 0 square   1 23 0.0);;
let sum_y2 = (sum_filtered_column pairs 1 square   1 23 0.0);;
let sum_xy = (sum_product_of_columns pairs 1 23 0.0);;

let denom = 23.0 *. sum_x2 -. (square sum_x);;
let slope = (23.0 *. sum_xy -. sum_x *. sum_y) /. denom;; 
